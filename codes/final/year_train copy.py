...
def multi_train(train,test,data,sampletime,epochs=10,labelsize=1,unlabelsize=5,tradeoff=1):
    ...
    return multi_initial_result,multi_final_result,multi_model1,multi_model2,multi_model3,multi_model4
...
def train_model(train,test,data,sampletimes=50,epochs=10,labelsize=1,unlabelsize=5,tradeoff=1):
    ...
    for sampletime in range(sampletimes):
        ...
        print("final: multi:1:%.4f,2:%.4f,3:%.4f,4:%.4f,self:1:%.4f,2:%.4f,3:%.4f,4:%.4f"%(multi_final_result[0,5],multi_final_result[1,5],multi_final_result[2,0.5],multi_final_result[3,5],self_final_result[0,5],self_final_result[1,5],self_final_result[2,5],self_final_result[3,5]))
...
image1_name=r"170211.tif"
image2_name=r"170402.tif"
image3_name=r"171009.tif"
image4_name=r"171218.tif"
label_path=r"G:\project\images\sentinal\process\labels"
label1_name=r"train170211.csv"
label2_name=r"train170402.csv"
label3_name=r"train171009.csv"
label4_name=r"train171218.csv"
map_dir=r"G:\project\images\sentinal\process\output\map\17"
csv_path=r"G:\project\images\sentinal\process\output\res\17"
epochs=1
sampletimes=50
tradeoff=1
n=5
...
# %%
def colormap():
    return mpl.colors.LinearSegmentedColormap.from_list('cmap', ['#EEE8AA','#FFFF00','#EE2C2C','#9A32CD','#00008B'],)
#%%
fig=plt.figure(figsize=(14,10),dpi=140)
y_proba=y_proba.reshape((653,772))
image=(y_proba-y_proba.min())/(y_proba.max()-y_proba.min())
plt.imshow(image,cmap=colormap())
plt.axis('off')
plt.colorbar()

#plt.annotate('mean %.2f'%m,xy=(8,8),xytext=(900,4),arrowprops=dict(facecolor='black'))
plt.tight_layout()
plt.savefig(r"G:\1.jpg")
# %%
