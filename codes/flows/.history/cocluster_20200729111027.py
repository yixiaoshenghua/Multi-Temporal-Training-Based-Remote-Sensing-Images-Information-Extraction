...
def run(resdf,datadf,x1,y1,x2,y2,model1,model2,x_test1,y_test1,x_test2,y_test2,c=1.0,size=5,epoch=20,selflearn=False,curepoch=0):
    if selflearn==False:
        for i in range(epoch):
            ...
            pred1=model1.predict(datadf[datadf['change1']==0].loc[:,features1])
            pred2=model2.predict(datadf[datadf['change1']==0].loc[:,features2])
            y_pred1=model1.predict_proba(datadf[datadf['change1']==0].loc[:,features1])
            y_pred2=model2.predict_proba(datadf[datadf['change1']==0].loc[:,features2])
            y1_pred=model1.predict(datadf.loc[:,features1])
            y2_pred=model2.predict(datadf.loc[:,features2])
        ...
        for i in range(epoch):
            ...
            resdf.at[ii,'sePDC']=calcPDC(datadf,y1_pred1,y2_pred)
            x1,y1,x2,y2,model1,model2=cotrain(datadf,x1,y1,x2,y2,model1,model2,c,size,selflearn)
            res1=classification_report(y_test1,y_pre1,
                                    target_names=['veg','water','building'],
                                    output_dict=True)
