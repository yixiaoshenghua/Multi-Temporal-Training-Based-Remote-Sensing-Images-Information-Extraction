...
class GradientBoostedDecisionTree:
    ...
    def fit(self, X, Y, X1, X2):
        ...
        Y_pred1 = np.zeros((N1, self.out_dims))
        Y_pred2 = np.zeros((N2, self.out_dims))
        ...
        for k in range(self.out_dims):
            ...
            Y_pred1[:,k] +=t.predict(X1)
            Y_pred2[:,k] += t.predict(X2)
            self.learners[0, k] = t
        ...
        for i in range(1, self.n_iter):
            for k in range(self.out_dims):
                ...
                y1_pred, y2_pred = Y_pred1[:,k], Y_pred1[:,k]
                co_neg_grad1 = -1 * loss.grad(y1_pred, y2_pred) 
