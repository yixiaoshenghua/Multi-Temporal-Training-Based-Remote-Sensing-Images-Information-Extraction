...
class GradientBoostedDecisionTree:
    ...
    def fit(self, X, Y, X_, Y_, X1, X2):
        ...
        if self.classifier:
            ...
            Y_= to_one_hot(Y_.flatten())
        else:
            Y = Y.reshape(-1, 1) if len(Y.shape) == 1 else Y
            Y_= Y_.reshape(-1,1) if len(Y_.shape) == 1 else Y_
        ...
        # fit the base estimator and fit the unlabel data 1,2
        Y_pred = np.zeros((N+N1, self.out_dims))
        Y_pred1 = np.zeros((N2, self.out_dims))

        for k in range(self.out_dims):
            t = loss.base_estimator()
            t_ = loss.base_estimator()
            t.fit(X, Y[:, k])
            t_.fit(X_, Y_[:, k])

            concate_X1=np.concatenate((X,X1),axis=0)
            Y_pred[:, k] += t.predict(concate_X1)

            Y_pred1[:,k] += t_.predict(X2)
            Y_pred2 = np.concatenate((Y,Y_pred1),axis=0)
