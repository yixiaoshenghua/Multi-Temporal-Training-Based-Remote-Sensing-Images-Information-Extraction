...
def run(resdf,datadf,x1,y1,x2,y2,model1,model2,x_test1,y_test1,x_test2,y_test2,c=1.0,size=5,epoch=20,selflearn=False,curepoch=0):
    if selflearn==False:
        i=0
        consistency_loss=10000000
        for i in range(epoch):
            ii=curepoch*epoch+i
            # 预训练模型
            x1_tmp,y1_tmp,x2_tmp,y2_tmp=x1.copy(),y1.copy(),x2.copy(),y2.copy()
            model1_tmp,model2_tmp=RandomForestClassifier(n_estimators=50),RandomForestClassifier(n_estimators=50)
            model1_tmp,model2_tmp=clone(model1),clone(model2)
            model1_tmp.fit(x1_tmp,y1_tmp.ravel())
            model2_tmp.fit(x2_tmp,y2_tmp.ravel())
            x1_tmp,y1_tmp,x2_tmp,y2_tmp,model1_tmp,model2_tmp=cotrain(datadf,x1_tmp,y1_tmp,x2_tmp,y2_tmp,model1_tmp,model2_tmp,c,size,selflearn)
            model1_tmp.fit(x1_tmp,y1_tmp.ravel())
            model2_tmp.fit(x2_tmp,y2_tmp.ravel())
            pred1=model1_tmp.predict(datadf[datadf['change1']==0].loc[:,features1])
            pred2=model2_tmp.predict(datadf[datadf['change1']==0].loc[:,features2])
            y_proba1=model1_tmp.predict_proba(datadf[datadf['change1']==0].loc[:,features1])
            y_proba2=model2_tmp.predict_proba(datadf[datadf['change1']==0].loc[:,features2])
            y1_pred=model1_tmp.predict(datadf.loc[:,features1])
            y2_pred=model2_tmp.predict(datadf.loc[:,features2])
            current_consistency_loss=calcConsistencyLoss(datadf,y_proba1,pred2,y_proba2,pred1)
            # 保证损失下降才继续
            j=0
            print(current_consistency_loss)
            while current_consistency_loss>=consistency_loss:
                j+=1
                x1_tmp,y1_tmp,x2_tmp,y2_tmp=x1.copy(),y1.copy(),x2.copy(),y2.copy()
                model1_tmp,model2_tmp=clone(model1),clone(model2)
                model1_tmp.fit(x1_tmp,y1_tmp.ravel())
        ...
        for i in range(epoch):
            ...
            resdf.at[ii,'seLoss']=consistency_loss
            print(consistency_loss)
            x1,y1,x2,y2=x1_tmp,y1_tmp,x2_tmp,y2_tmp
            model1,model2=clone(model1_tmp),clone(model2_tmp)
            model1.fit(x1,y1)
...
def runModel(datadf,x1,y1,x2,y2,x_test1,y_test1,x_test2,y_test2,c=1.0,size=5,epoch=20,labelsize=1,sampletimes=10):
    resdf=pd.DataFrame(np.zeros((sampletimes*epoch,20)),
...
def calcConsistencyLoss(datadf,y_proba1,y2_pred,y_proba2,y1_pred):
    unlabeled_loss1=log_loss(y2_pred,y_proba1,labels=[0,1,2])
    unlabeled_loss2=log_loss(y1_pred,y_proba2,labels=[0,1,2])
    loss=unlabeled_loss1+unlabeled_loss2
    return loss
...
for i in range(1,4):
    ...
    resdf.to_csv(r"E:\project\images\sentinal\process\output\res\resdf_consistencyLossOptimize__label"+str(i)+"_unlabel5_epoch20_c_1_0.csv",index=None)

# %%
data=[]
for i in range(1,4):
    data.append(np.loadtxt(open(r"E:\project\images\sentinal\process\output\res\resdf_consistency_YHLO_label"+str(i)+"_unlabel5_epoch20_c_1_0.csv",'r'),delimiter=',',skiprows=1,dtype=np.float32))
...
plt.title("Consistency Loss between classifiers(unlabeled data 5 c 1.0)",fontsize=20)
plt.tight_layout()
plt.savefig(r"E:\project\images\sentinal\process\output\YHLO_label5unlabel5epoch20c1_0Coloss123.jpg")
plt.show()
