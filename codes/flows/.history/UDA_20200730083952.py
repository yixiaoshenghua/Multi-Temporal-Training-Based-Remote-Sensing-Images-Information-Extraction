...
class consistencyLoss(torch.nn.Module):
    ...
    def forward(self,unlabeled_yProba_train1,unlabeled_yProba_train2):
        '''
        params: 
        ...
        unlabeled_data_loss=F.kl_div(unlabeled_yProba_train2.log(),unlabeled_yProba_train1,reduction='sum')
...
class CoModel:
    def __init__(self,lamda,labeled_X_train1,labeled_yTrue_train1,labeled_X_train2,labeled_yTrue_train2,unlabeled_X_train1,unlabeled_X_train2):
        self.net1=dnn(n_feature=9,n_hidden1=32,n_hidden2=64,n_hidden3=96,n_hidden4=32,n_hidden5=16,n_ouput=3)
        self.net2=dnn(n_feature=9,n_hidden1=32,n_hidden2=64,n_hidden3=96,n_hidden4=32,n_hidden5=16,n_ouput=3)
        self.lamda=lamda
    ...
    def fit(self,epochs=20):
        ...
        loader2 = Data.DataLoader(dataset=train_dataset2,batch_size=150,shuffle=True)
        ...
        for i in range(epochs):
            ...
            for step, (labeled_batch_X_train1,labeled_batch_yTrue_train1) in enumerate(loader1):
                ...
                unlabeled_yProba_train2=self.predict_proba(self.unlabeled_X_train2,model_id=1)
                loss1=loss_func1(unlabeled_yProba_train1,unlabeled_yProba_train2)
                print(step,loss1)
            ...
            for step, (labeled_batch_X_train2,labeled_batch_yTrue_train2) in enumerate(loader2):
                ...
                unlabeled_yProba_train1=self.predict_proba(self.unlabeled_X_train1,model_id=2)
                unlabeled_yProba_train2=self.predict_proba(self.unlabeled_X_train2,model_id=2)
                loss2=loss_func2(unlabeled_yProba_train2,unlabeled_yProba_train1)
                print(step,loss2)
                optimizer2.zero_grad()
                loss2.backward()
                optimizer2.step()

            self.update(size=20,c=0.3)
    ...
    def update(self,size=20,c=1.0):
        unlabeled_yProba_train1=self.predict_proba(self.unlabeled_X_train1).detach().numpy().max(axis=1)
        unlabeled_yProba_train2=self.predict_proba(self.unlabeled_X_train2).detach().numpy().max(axis=1)
        print(unlabeled_yProba_train1,unlabeled_yProba_train2)
        unlabeled_yPred_train1=self.predict_class(self.unlabeled_X_train1).detach().numpy()
        unlabeled_yPred_train2=self.predict_class(self.unlabeled_X_train2).detach().numpy()
        ...
        for i in range(3):
            ...
            # print(class_candidate[i]['unlabeled_yProba_train1'].mean(),class_candidate[i]['unlabeled_yProba_train2'].mean())
            ...
            index1=np.array(class_candidate[i].sort_values(by='unlabeled_yProba_train1',ascending=False).index)
            index2=np.array(class_candidate[i].sort_values(by='unlabeled_yProba_train2',ascending=False).index)
            index=np.array([x for x in index1 if x in index2])
            print("index:",index)
            sample1=np.random.randint(int(index.shape[0])//2,size=int(size))
            sample2=np.random.randint(int(index.shape[0])//2,size=int(size))
            can1=index1[sample1]
            can2=index2[sample2]
            ...
            y1=np.concatenate((y1,class_candidate[i].loc[can1,['unlabeled_yPred_train1']].values.reshape((-1))),axis=0)
            y2=np.concatenate((y2,class_candidate[i].loc[can2,['unlabeled_yPred_train2']].values.reshape((-1))),axis=0)
...
# %%
comodel=CoModel(0.1,labeled_X_train1,labeled_yTrue_train1,labeled_X_train2,labeled_yTrue_train2,unlabeled_X_train1,unlabeled_X_train2)
