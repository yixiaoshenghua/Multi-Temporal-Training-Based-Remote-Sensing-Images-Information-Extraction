...
def cotrain(datadf,x1,y1,x2,y2,model1,model2,c=1.0,size=5,selflearn=False):
    ...
    if selflearn==False:
        # 找到guessclass一致区域的数据
        datadf_candidate=datadf[datadf['change1']==0][datadf['change2']==0][datadf['guessClass1']==datadf['guessClass2']]
        # 标签一致区域找到置信度最高的区域
        ...
        for i in range(n):
            ...
            can1=index1[sample1]
            can2=index2[sample2]
            datadf.loc[can1,'change1']=1
            datadf.loc[can2,'change2']=1
            ...
            y1=np.concatenate((y1,class_candidate[i].loc[can1,['guessClass1']].values.ravel()),axis=0)
            y2=np.concatenate((y2,class_candidate[i].loc[can2,['guessClass2']].values.ravel()),axis=0)
    else:
        class_candidate1=[datadf[datadf['change1']==0][datadf['guessClass1']==i] for i in range(n)]
        class_candidate2=[datadf[datadf['change2']==0][datadf['guessClass2']==i] for i in range(n)]
        for i in range(n):
            ...
            y1=np.concatenate((y1,class_candidate1[i].loc[can1,['guessClass1']].values.ravel()),axis=0)
            y2=np.concatenate((y2,class_candidate2[i].loc[can2,['guessClass2']].values.ravel()),axis=0)
    return x1,y1,x2,y2,model1,model2

def run(resdf,datadf,x1,y1,x2,y2,model1,model2,x_test1,y_test1,x_test2,y_test2,c=1.0,size=5,epoch=10,selflearn=False,curepoch=0):
    if selflearn==False:
        ...
        for i in range(epoch):
            ...
            resdf.iat[ii,19],resdf.iat[ii,20],resdf.iat[ii,21],resdf.iat[ii,22],resdf.iat[ii,23],resdf.iat[ii,24]=res2['veg']['f1-score'],res2['shadow']['f1-score'],res2['water']['f1-score'],res2['road']['f1-score'],res2['building']['f1-score'],res2['weighted avg']['f1-score']
            if i==0 or i==epochs:
                print("score: f1:%.5f f2:%.5f"%(res1['weighted avg']['f1-score'],res2['weighted avg']['f1-score']))
...
def runModel(datadf,x1,y1,x2,y2,x_test1,y_test1,x_test2,y_test2,c=1.0,size=5,epoch=20,labelsize=1,sampletimes=10):
    resdf=pd.DataFrame(np.zeros((sampletimes*epoch,26)),
    ...
    for i in range(sampletimes):
        ...
        cv2.imwrite(r"G:\project\images\sentinal\process\output\map\161103_170211\co_label_"+str(labelsize)+"unlabel_"+str(size)+"class1_"+str(i)+".tif",y_pred1)
        y_pred2=model2.predict(data2reshape).astype(np.uint8).reshape((653,772))
        cv2.imwrite(r"G:\project\images\sentinal\process\output\map\161103_170211\co_label_"+str(labelsize)+"unlabel_"+str(size)+"class2_"+str(i)+".tif",y_pred2)
        print("self_%d epoch"%i)
        ...
        cv2.imwrite(r"G:\project\images\sentinal\process\output\map\161103_170211\self_label_"+str(labelsize)+"unlabel_"+str(size)+"class1_"+str(i)+".tif",y_pred3)
        y_pred4=model2.predict(data2reshape).astype(np.uint8).reshape((653,772))
        cv2.imwrite(r"G:\project\images\sentinal\process\output\map\161103_170211\self_label_"+str(labelsize)+"unlabel_"+str(size)+"class2_"+str(i)+".tif",y_pred4)
    return resdf
...
def color(y_pred):
    ...
    cmap=np.array([[0,255,0],[135,138,128],[230,224,176],[0,0,255],[255,0,0],[0,255,255]])
    for i in range(6):
...
def calcConsistencyLoss(datadf,y_pred1,y2_pred,lamda=1.0):
    ...
    unlabeled_loss=log_loss(y2_pred,y_pred1,labels=[0,1,2,3,4,5])
...
def random_train_test_split(data,size,train_rate=0.2,test_rate=0.8):
    ...
    for i in range(n):
        ...
        X_train[i*train_class_size:(i+1)*train_class_size,:],X_test[i*test_class_size:(i+1)*test_class_size,:],y_train[i*train_class_size:(i+1)*train_class_size],y_test[i*test_class_size:(i+1)*test_class_size] = train_test_split(tmp[:,:-1],tmp[:,-1:].ravel(),test_size=train_rate,random_state=1023)
...
loc1=label1[:,:2]
loc2=label2[:,:2]
train1=addY(loc1,data1total,[1000]*5)
train2=addY(loc2,data2total,[1000]*5)

x_train1,x_test1,y_train1,y_test1=random_train_test_split(train1,size=1000,train_rate=0.2,test_rate=0.8)
x_train2,x_test2,y_train2,y_test2=random_train_test_split(train2,size=1000,train_rate=0.2,test_rate=0.8)
...
unmask=returnUnlabeledData(data1total,data2total)
...
data1df['change1']=np.ones((data1reshape.shape[0]))
data2df['change2']=np.ones((data2reshape.shape[0]))
data1df.loc[unmask,'change1']=0
data2df.loc[unmask,'change2']=0
...
for i in range(1,9,2):
    ...
    for size in range(5,55,5):
        print('unlabel:',size)
        resdf=runModel(datadf,x_train1,y_train1,x_train2,y_train2,x_test1,y_test1,x_test2,y_test2,c=1.0,size=size,epoch=10,labelsize=i,sampletimes=10)
        resdf.to_csv(r"G:\project\images\sentinal\process\output\res\\"+str(i)+"_unlabel_"+str(size)+"_c_1_0.csv",index=None)

# %%
data=[]
for i in range(1,6,1):
    data.append(np.loadtxt(open(r"G:\project\images\sentinal\process\output\res\resdf_multi_label"+str(i)+"_unlabel5_epoch20_c_1_0.csv",'r'),delimiter=',',skiprows=1,dtype=np.float32))
res=np.zeros((5,20,34))
for i in range(5):
    for j in range(10):
...
i=0
plt.plot(epochs,res[i][:,0],color='green',marker='^',linestyle='-')
plt.plot(epochs,res[i][:,7],color='green',marker='o',linestyle='-')
plt.plot(epochs,res[i][:,14],color='black',marker='^',linestyle='-.')
plt.plot(epochs,res[i][:,21],color='black',marker='o',linestyle='-.')
plt.ylim(0.6,1)
plt.xlim(0,20)
plt.xticks(epochs)
...
plt.title("labeldata "+str(i+1)+" unlabeldata 5 c 1.0 veg",fontsize=20)
plt.tight_layout()
plt.gca().xaxis.set_major_formatter(ticker.FormatStrFormatter('%d'))
plt.savefig(r"E:\project\images\sentinal\process\output\multi_label"+str(i+1)+"unlabel5Epoch20c_1_0_veg.jpg")
plt.show()
