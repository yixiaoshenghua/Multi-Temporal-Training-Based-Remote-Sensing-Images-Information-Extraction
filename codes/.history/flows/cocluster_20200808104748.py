...
def addY(loc,data,nums):#data3维
    ...
    y=np.array([0]*nums[0]+[1]*nums[1]+[2]*nums[2]+[3]*nums[3]+[4]*nums[4]+[5]*nums[5]).reshape((sum(nums),1))
...
def cotrain(datadf,x1,y1,x2,y2,model1,model2,c=1.0,size=5,selflearn=False):
    ...
    if selflearn==False:
        ...
        # 标签一致区域找到置信度最高的区域
        class_candidate=[datadf_candidate[datadf_candidate['guessClass1']==i] for i in range(6)]
        # 找到两期置信度都高的区域更新训练集
        for i in range(6):
            # tradeoff
        ...
        class_candidate1=[datadf[datadf['guessClass1']==i] for i in range(6)]
        class_candidate2=[datadf[datadf['guessClass2']==i] for i in range(6)]
        for i in range(6):
    ...
    if selflearn==False:
        ...
        for i in range(epoch):
            ...
            res2=classification_report(y_test2,y_pre2,
                                    target_names=['trees','shadows','water','roads','houses','fallow'],
                                    output_dict=True)
            resdf.iat[ii,14],resdf.iat[ii,15],resdf.iat[ii,16],resdf.iat[ii,17],resdf.iat[ii,18],resdf.iat[ii,19],resdf.iat[ii,20]=res1['trees']['f1-score'],res1['shadows']['f1-score'],res1['water']['f1-score'],res1['roads']['f1-score'],res1['houses']['f1-score'],res1['fallow']['f1-score'],res1['weighted avg']['f1-score']
            resdf.iat[ii,21],resdf.iat[ii,22],resdf.iat[ii,23],resdf.iat[ii,24],resdf.iat[ii,25],resdf.iat[ii,26],resdf.iat[ii,27]=res2['trees']['f1-score'],res2['shadows']['f1-score'],res2['water']['f1-score'],res2['roads']['f1-score'],res2['houses']['f1-score'],res2['fallow']['f1-score'],res2['weighted avg']['f1-score']
            if i%5==0:
                print("score: f1:%.5f f2:%.5f"%(res1['weighted avg']['f1-score'],res2['weighted avg']['f1-score']))
...
def runModel(datadf,x1,y1,x2,y2,x_test1,y_test1,x_test2,y_test2,c=1.0,size=5,epoch=20,labelsize=1,sampletimes=10):
    resdf=pd.DataFrame(np.zeros((sampletimes*epoch,34)),
                    columns=['17co'+str(labelsize)+'trees','17co'+str(labelsize)+'shadows','17co'+str(labelsize)+'water','17co'+str(labelsize)+'roads','17co'+str(labelsize)+'houses','17co'+str(labelsize)+'fallow','17co'+str(labelsize)+'total',
                            '19co'+str(labelsize)+'trees','19co'+str(labelsize)+'shadows','19co'+str(labelsize)+'water','19co'+str(labelsize)+'roads','19co'+str(labelsize)+'houses','19co'+str(labelsize)+'fallow','19co'+str(labelsize)+'total',
                            '17se'+str(labelsize)+'trees','17se'+str(labelsize)+'shadows','17se'+str(labelsize)+'water','17se'+str(labelsize)+'roads','17se'+str(labelsize)+'houses','17se'+str(labelsize)+'fallow','17se'+str(labelsize)+'total',
                            '19se'+str(labelsize)+'trees','19se'+str(labelsize)+'shadows','19se'+str(labelsize)+'water','19se'+str(labelsize)+'roads','19se'+str(labelsize)+'houses','19se'+str(labelsize)+'fallow','19se'+str(labelsize)+'total',
    ...
    for i in range(sampletimes):
        ...
        print("%d epoch"%i)
        base1=svm.SVC(probability=True)
        base2=svm.SVC(probability=True)
        model1=BaggingClassifier(base_estimator=base1, n_estimators=50, random_state=7)
        model2=BaggingClassifier(base_estimator=base2, n_estimators=50, random_state=7)
        ...
        print("%d epoch"%i)
        base3=svm.SVC(probability=True)
        base4=svm.SVC(probability=True)
        model3=BaggingClassifier(base_estimator=base3, n_estimators=50, random_state=7)
        model4=BaggingClassifier(base_estimator=base4, n_estimators=50, random_state=7)
...
loc1=label1[:,:2]
loc2=label2[:,:2]
train1=addY(loc1,data1total,[20]*6)
train2=addY(loc2,data2total,[20]*6)
...
for i in range(1,5,2):
    resdf=runModel(datadf,x1,y1,x2,y2,x_test1,y_test1,x_test2,y_test2,c=1.0,size=2*i,epoch=20,labelsize=i,sampletimes=10)
    resdf.to_csv(r"E:\project\images\sentinal\process\output\res\resdf_svm_multi_label"+str(i)+"_unlabel_2times_epoch20_c_1_0.csv",index=None)

# %%
data=[]
for i in range(1,6,1):
    data.append(np.loadtxt(open(r"E:\project\images\sentinal\process\output\res\resdf_multi_label"+str(i)+"_unlabel5_epoch20_c_1_0.csv",'r'),delimiter=',',skiprows=1,dtype=np.float32))
...
plt.title("labeldata "+str(i+1)+" unlabeldata 5 c 1.0 trees",fontsize=20)
plt.tight_layout()
plt.gca().xaxis.set_major_formatter(ticker.FormatStrFormatter('%d'))
plt.savefig(r"E:\project\images\sentinal\process\output\multi_label"+str(i+1)+"unlabel5Epoch20c_1_0_trees.jpg")
plt.show()
