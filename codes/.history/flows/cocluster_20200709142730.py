...
# load label
lab1=np.loadtxt(open(r"E:\project\images\sentinal\process\label\label17.csv","r"),delimiter=',',skiprows=25,dtype=np.int)
lab2=np.loadtxt(open(r"E:\project\images\sentinal\process\label\label19.csv","r"),delimiter=',',skiprows=25,dtype=np.int)
testloc1=lab1[:,:2]
testloc2=lab2[:,:2]
test1=addY(testloc1,data1total,[200,200,200,200])
test2=addY(testloc2,data2total,[200,200,200,200])
x_test1 = test1[:,:-1]
y_test1 = test1[:,-1]
x_test2 = test2[:,:-1]
y_test2 = test2[:,-1]
...
sescores2=[]
for i in range(10):
    x1_train,y1_train=selectSample(x1,y1,(i+1)*2)
    x2_train,y2_train=selectSample(x2,y2,(i+1)*2)
    print("%d epoch"%i)
    ...
    model1,model2,coscore1,coscore2=run(datadf,x_train1,y_train1,x_train2,y_train2,model1,model2,x_test1,y_test1,x_test2,y_test2,size=2,epoch=40)
    y_pred1=model1.predict(data1reshape).astype(np.int)
    show1=color(y_pred1)
    cv2.imwrite(r"E:\project\images\sentinal\process\output\cotrain\cosample"+str((i+1)*2)+"epoch40size2class17.jpg",show1)
    y_pred2=model2.predict(data2reshape).astype(np.int)
    show2=color(y_pred2)
    cv2.imwrite(r"E:\project\images\sentinal\process\output\cotrain\cosample"+str((i+1)*2)+"epoch40size2class19.jpg",show2)
    ...
    model3,model4,sescore1,sescore2=run(datadf,x_train3,y_train3,x_train4,y_train4,model3,model4,x_test1,y_test1,x_test2,y_test2,size=2,epoch=40,selflearn=True)
    y_pred3=model3.predict(data1reshape).astype(np.int)
    show3=color(y_pred3)
    cv2.imwrite(r"E:\project\images\sentinal\process\output\cotrain\selfsample"+str((i+1)*2)+"epoch40size2class17.jpg",show3)
    y_pred4=model2.predict(data2reshape).astype(np.int)
    show4=color(y_pred4)
    cv2.imwrite(r"E:\project\images\sentinal\process\output\cotrain\selfsample"+str((i+1)*2)+"epoch40size2class19.jpg",show4)
