...
class model:
    ...
    def fit(self,labeled_X_train1,labeled_yTrue_train1,unlabeled_X_train1,unlabeled_X_train2,epochs=20):
        train_dataset = Data.TensorDataset(data_tensor=labeled_X_train1, target_tensor=labeled_yTrue_train1)
        loader = Data.DataLoader(dataset=train_dataset,batch_size=20,shuffle=True,num_workers=2)
        optimizer=torch.optim.SGD(self.net.parameters(),lr=0.02)
        loss_func=consistencyLoss(lamda=self.lamda)
        for i in range(epochs):
            for step, (labeled_batch_X_train1, labeled_batch_yTrue_train1) in enumerate(loader):
            labeled_batch_yProba_train1=self.net(labeled_batch_X_train1)
            unlabeled_yProba_train1=self.net(unlabeled_X_train1)
            unlabeled_yPred_train2=torch.max(self.net(unlabeled_X_train2),1)[1]
            loss=loss_func(labeled_batch_yProba_train1,labeled_batch_yTrue_train1,unlabeled_yProba_train1,unlabeled_yPred_train2)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()

    def predict_proba(self,X_test):
        yProba_test=self.net(X_test)
        return yProba_test

    def predict_class(self,X_test):
        yPred_test=torch.max(self.net(X_test),1)[1]
        return yPred_test
