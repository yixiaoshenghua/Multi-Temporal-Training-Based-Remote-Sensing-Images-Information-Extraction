...
class CoModel:
    ...
    def fit(self,epochs=20):
        ...
        optimizer2=torch.optim.SGD(self.net.parameters(),lr=0.02)
        loss_func2=consistencyLoss(lamda=self.lamda)
        for i in range(epochs):
            for step, (labeled_batch_X_train1, labeled_batch_yTrue_train1),(labeled_batch_X_train2,labeled_batch_yTrue_train2) in enumerate(zip(loader1,loader2)):
                labeled_batch_yProba_train1=self.net(labeled_batch_X_train1)
                ...
                loss1=loss_func1(labeled_batch_yProba_train1,self.labeled_batch_yTrue_train1,unlabeled_yProba_train1,unlabeled_yPred_train2)
                optimizer1.zero_grad()
                loss1.backward()
                optimizer1.step()
                labeled_batch_yProba_train2=self.net(labeled_batch_X_train2)
                unlabeled_yPred_train1=self.predict_pred(self.unlabeled_X_train1,model_id=2)
                unlabeled_yProba_train2=self.predict_proba(self.unlabeled_X_train2,model_id=2)
                loss2=loss_func2(labeled_batch_yProba_train2,self.labeled_batch_yTrue_train2,unlabeled_yProba_train1,unlabeled_yPred_train2)
    ...
    def score(self,X_test,yTrue_test):
        yPred_test=self.predict_class(X_test)
...
yPred_test=model1.predict_class(x_test1)
score=classification_report(yTrue_test1,yPred_test,
                                    target_names=['veg','water','building'],
                                    output_dict=True)
scores=np.array([
            score['veg']['f1-score'],score['water']['f1-score'],score['building']['f1-score'],
            f1_score(yTrue_test1,yPred_test,average='weighted')
        ])
# %%
scores
