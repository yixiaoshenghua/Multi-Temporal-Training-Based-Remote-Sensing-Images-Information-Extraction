...
#%%
for i in range(1,6):
    resdf=runModel(datadf,x1,y1,x2,y2,x_test1,y_test1,x_test2,y_test2,c=1.2,size=5,epoch=20,labelsize=i,sampletimes=10)
    resdf.to_csv(r"E:\project\images\sentinal\process\output\res\resdf_label"+str(i)+"_unlabel5_epoch20_c_1_2.csv",index=None)

# %%
data=[]
for i in range(1,6):
    data.append(np.loadtxt(open(r"E:\project\images\sentinal\process\output\res\resdf_label"+str(i)+"_unlabel10_epoch20.csv",'r'),delimiter=',',skiprows=1,dtype=np.float32))
res=np.zeros((5,20,18))
for i in range(5):
    for j in range(10):
...
plt.plot(epochs,res[4][:,0],color='black',marker='^',linestyle='-')
plt.plot(epochs,res[4][:,4],color='black',marker='o',linestyle='-')
plt.plot(epochs,res[4][:,8],color='black',marker='^',linestyle='-.')
plt.plot(epochs,res[4][:,12],color='black',marker='o',linestyle='-.')
plt.ylim(0.7,1)
plt.xlim(0,20)
plt.xticks(epochs)
...
plt.title("labeldata 5 unlabeldata 5 (building)",fontsize=20)
plt.tight_layout()
plt.gca().xaxis.set_major_formatter(ticker.FormatStrFormatter('%d'))
plt.savefig(r"E:\project\images\sentinal\process\output\label5unlabel10Epoch20veg.jpg")
plt.show()
...
epochs=np.arange(1,21,dtype=np.int)
plt.figure(figsize=(10,8))
plt.plot(epochs,res[0][:,16],color='black',marker='^',linestyle='-.')
plt.plot(epochs,res[1][:,16],color='black',marker='*',linestyle='-.')
plt.plot(epochs,res[2][:,16],color='black',marker='+',linestyle='-.')
plt.plot(epochs,res[3][:,16],color='black',marker='o',linestyle='-.')
plt.plot(epochs,res[4][:,16],color='black',marker='v',linestyle='-.')
plt.ylim(0.2,0.5)
plt.xlim(0,20)
plt.xticks(epochs)
plt.ylabel('PDC',fontsize=20)
plt.xlabel("epoch",fontsize=20)
plt.legend(['labeldata 1','labeldata 2','labeldata 3','labeldata 4','labeldata 5'],fontsize=16)
plt.title("Prediction differences between classifiers(co-train)",fontsize=20)
plt.tight_layout()
plt.savefig(r"E:\project\images\sentinal\process\output\label5unlabel5epoch20c1_0coPDC12345.jpg")
plt.show()
