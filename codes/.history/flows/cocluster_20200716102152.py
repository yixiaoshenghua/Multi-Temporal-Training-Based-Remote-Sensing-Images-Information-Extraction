...
def run(resdf,datadf,x1,y1,x2,y2,model1,model2,x_test1,y_test1,x_test2,y_test2,c=1.0,size=50,epoch=20,selflearn=False):
    if selflearn==False:
        for i in range(epoch):
            ...
            resdf[i]['coPDC']=calcPDC(datadf,pred1,pred2)
            print('copdc',resdf[i]['coPDC'])
            ...
            resdf.iat[i,0],resdf.iat[i,1],resdf.iat[i,2],resdf.iat[i,3]=res1['veg']['f1-score'],res1['water']['f1-score'],res1['building']['f1-score'],f1
            resdf.iat[i,4],resdf.iat[i,5],resdf.iat[i,6],resdf.iat[i,7]=res2['veg']['f1-score'],res2['water']['f1-score'],res2['building']['f1-score'],f2
            if i%10==0:
        ...
        for i in range(epoch):
            ...
            pred1=model1.predict(datadf.loc[:,features1])
            pred2=model2.predict(datadf.loc[:,features2])
            resdf[i]['sePDC']=calcPDC(datadf,pred1,pred2)
            x1,y1,x2,y2,model1,model2=cotrain(datadf,x1,y1,x2,y2,model1,model2,c,size,selflearn)
            res1=classification_report(y_test1,y_pre1,
                                    target_names=['veg','water','building'],
                                    output_dict=True)
            ...
            f1=f1_score(y_test1, y_pre1, average='weighted')
            f2=f1_score(y_test2, y_pre2, average='weighted')
            resdf.iat[i,8],resdf.iat[i,9],resdf.iat[i,10],resdf.iat[i,11]=res1['veg']['f1-score'],res1['water']['f1-score'],res1['building']['f1-score'],f1
            resdf.iat[i,12],resdf.iat[i,13],resdf.iat[i,14],resdf.iat[i,15]=res2['veg']['f1-score'],res2['water']['f1-score'],res2['building']['f1-score'],f2
            if i%10==0:
                print("score: f1:%.5f f2:%.5f"%(f1,f2))
...
def calcPDC(datadf,pred1,pred2):
    ...
    for i in range(unchangedf.shape[0]):
        #print(it)
        if pd[unchangedf.loc[i,'row1'],unchangedf.loc[i,'col1']]!=0:
            pdc+=1
    return pdc/Nu
#%%
# prepare data
...
#%%
resdf1=pd.DataFrame(np.zeros((10,18)),
...
for i in range(10):
    ...
    model1,model2,resdf1=run(resdf1,datadf,x_train1,y_train1,x_train2,y_train2,model1,model2,x_test1,y_test1,x_test2,y_test2,size=5,epoch=20)
    ...
    model3,model4,resdf1=run(resdf1,datadf,x_train3,y_train3,x_train4,y_train4,model3,model4,x_test1,y_test1,x_test2,y_test2,size=5,epoch=20,selflearn=True)
