...
class consistencyLoss(torch.nn.Module):
    ...
    def forward(self,labeled_yProba_train1,labeled_yTrue_train1,unlabeled_yProba_train1,unlabeled_yPred_train2):
        ...
        print(labeled_yProba_train1.shape,labeled_yTrue_train1.shape,unlabeled_yProba_train1.shape,unlabeled_yPred_train2.shape)
...
class model:
    ...
    def fit(self,labeled_X_train1,labeled_yTrue_train1,unlabeled_X_train1,unlabeled_X_train2,epochs=20):
        ...
        for i in range(epochs):
            labeled_yProba_train1=self.net(labeled_X_train1)
            unlabeled_yProba_train1=self.net(unlabeled_X_train1)
            unlabeled_yPred_train2=torch.max(self.net(unlabeled_X_train2),1)[1]
            loss=loss_func(labeled_yProba_train1,labeled_yTrue_train1,unlabeled_yProba_train1,unlabeled_yPred_train2)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    def score(self,X_test,yTrue_test):
        ...
        scores=np.array([
            ...
            f1_score(yTrue_test,yPred_test)
        ],dtype==np.float32)
        return scores
